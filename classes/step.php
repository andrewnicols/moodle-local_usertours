...
class step {
    ...
    protected function fetch($id) {
        ...
        return $this->reload_from_record(
            $DB->get_record('usertours_steps', array('id' => $id))
        );
    }
    ...
    public function persist($force = false) {
        ...
        if ($this->id) {
            ...
            $DB->update_record('usertours_steps', $record);
        } else {
            $this->calculate_sortorder();
            $record = $this->to_record();
            unset($record->id);
            $this->id = $DB->insert_record('usertours_steps', $record);
        }
    ...
    public function remove() {
        ...
        $DB->delete_records('usertours_steps', array('id' => $this->id));
    ...
    public function add_config_field_to_form(\MoodleQuickForm $mform, $key) {
        if (!$this->get_target()->is_setting_forced($key)) {
            ...
            $options[configuration::TOURDEFAULT] = get_string('defaultvalue', 'local_usertours', $options[$default]);
            $options = array_reverse($options, true);
