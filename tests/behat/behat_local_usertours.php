 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_local_usertours extends behat_base {
    ...
    public function i_add_a_new_user_tour_with(TableNode $table) {
        $this->execute('behat_local_usertours::i_open_the_user_tour_settings_page');
        $this->execute('behat_general::click_link', get_string('newtour', 'local_usertours'));
    ...
    public function i_add_steps_to_the_named_tour($tourname, TableNode $table) {
        ...
        foreach ($table->getHash() as $step) {
            $this->execute('behat_general::click_link', get_string('newstep', 'local_usertours'));
            $this->execute('behat_forms::i_set_the_field_to', ['targettype', $this->escape($step['targettype'])]);
            $this->execute('behat_forms::press_button', get_string('next', 'local_usertours'));
