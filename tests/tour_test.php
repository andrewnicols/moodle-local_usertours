...
class tour_testcase extends advanced_testcase {
    ...
    public function get_config_provider() {
        ...
        return [
                ...
                'Configuration value' => [
                        ...
                        'placement',
                        null,
                        \local_usertours\configuration::get_default_value('placement'),
                    ],
    ...
    public function test_remove_persisted() {
        ...
        $DB->expects($this->once())
            ->method('delete_records')
            ->with($this->equalTo('usertours_tours'), $this->equalTo(['id' => $id]))
            ->willReturn(null)
            ;

        $DB->expects($this->once())
            ->method('get_records')
            ->with($this->equalTo('usertours_tours'), $this->equalTo(null))
            ->willReturn([])
            ;
    ...
    public function test_reset_step_sortorder() {
        ...
        for ($i = 4; $i >= 0; $i--) {
            ...
            $expectations[] = [$this->equalTo('usertours_steps'), $this->equalTo('sortorder'), 4 - $i, ['id' => $id]];
        }
    ...
    public function test_should_show_for_user($completiondate, $requesteddate, $updateddate, $expectation) {
        ...
        if ($completiondate !== null) {
            set_user_preference(\local_usertours\tour::TOUR_LAST_COMPLETED_BY_USER . $id, $completiondate);
        }

        if ($requesteddate !== null) {
            set_user_preference(\local_usertours\tour::TOUR_REQUESTED_BY_USER . $id, $requesteddate);
        }
    ...
    public function get_tour_key_provider() {
        ...
        return [
            'No initial values' => [
                    ...
                    true,
                    null,
                    sprintf('local_usertours_%d_%s', $id, $time),
                ],
            ...
            'Initial tour time, with user reset higher' => [
                    ...
                    false,
                    $time + DAYSECS,
                    sprintf('local_usertours_%d_%s', $id, $time + DAYSECS),
                ],
        ];
    }
    ...
    public function test_get_tour_key($id, $getconfig, $setconfig, $willpersist, $userpref, $expectation) {
        ...
        if ($userpref !== null) {
            set_user_preference(\local_usertours\tour::TOUR_REQUESTED_BY_USER . $id, $userpref);
        }
    ...
    public function test_requested_user_reset() {
        ...
        $this->assertGreaterThanOrEqual($time, get_user_preferences(\local_usertours\tour::TOUR_REQUESTED_BY_USER . $id));
    }
    ...
    public function test_mark_user_completed() {
        ...
        $this->assertGreaterThanOrEqual($time, get_user_preferences(\local_usertours\tour::TOUR_LAST_COMPLETED_BY_USER . $id));
    }
     ...
     * @dataProvider sortorder_first_last_provider
     */
    public function test_is_last_tour_calculated($sortorder, $islast, $total, $islast) {
        $tour = new \local_usertours\tour();
